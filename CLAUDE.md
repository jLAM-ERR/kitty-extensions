...
**Pane placement.** `kitty_launch_split` pins every split to the lead session's tab with `--match window_id:$KITTY_WINDOW_ID`. Without it, `launch` splits whatever kitty window is *currently active*, so teammates land in whichever tab the user is looking at rather than the lead's tab. `--keep-focus` keeps the user where they are.

## How the shim is structured

`tmux`'s `main()` dispatches each invocation through three buckets:

- **`HANDLERS`** — tmux subcommands that map to real kitty actions (`split-window`→`launch`, `send-keys`→`send-text`/`send-key`, `capture-pane`→`get-text`, `kill-pane`→`close-window`, `select-pane`→`focus-window`(+`set-window-title` for `-T`), `respawn-pane`→`send-text` (runs the real teammate command in the placeholder pane's shell), `display-message`; plus the external-session set `new-session`/`new-window`/`has-session`/`list-panes`/`list-windows`). To support a new behavior, write a `cmd_*` handler and register it here.
