...
- **Programs restart fresh** — no scrollback or process state; a pane mid-task
  re-runs its command. To restore certain programs (e.g. `claude`, `ssh`, dev
  servers) as a plain shell in the right cwd instead, add their basenames to the
  `SHELLS` set in `save-session.py`, or drop the program-relaunch entirely by
  removing the `-- <cmd>` emission in `render()`.
